...
// Mix the extracted action handlers (js/checklists/actions.js) onto the
// Checklists object so they appear as Checklists._actionXxx methods. The
// dispatcher in _handleAction looks these up by name — no other call sites
// need to change. The script-tag order in index.html ensures
// window.ChecklistActions is defined before this runs.
if (window.ChecklistActions) {
  Object.assign(Checklists, window.ChecklistActions);
} else {
  console.error('[Checklists] ChecklistActions module did not load — action handlers will fail.');
}
