...
router.post('/lendingpad', async (req, res, next) => {
  try {
    ...
    if (loan.loan_id) {
      ...
      if (existing.length > 0) {
        const updates = Object.entries(loan)
          .filter(([, v]) => v !== null)
          .map(([k]) => `${k} = ?`);
        const values = Object.entries(loan)
          .filter(([, v]) => v !== null)
          .map(([, v]) => v);
        values.push(existing[0].id);

        await db.query(
          `UPDATE lendingpad_loans SET ${updates.join(', ')}, updated_at = NOW() WHERE id = ?`,
          values
        );
