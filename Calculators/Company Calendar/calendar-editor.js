        ...
        <section class="schedule-editor" role="dialog" aria-modal="true" aria-labelledby="scheduleEditorTitle">
          <form class="schedule-editor-form">
            <div class="editor-head">
              ...
              <button class="icon-btn" type="button" aria-label="Close editor" data-editor-close>&times;</button>
            </div>
            <div class="editor-grid">
              <label>
                <span>Employee ID</span>
                <input name="user_id" type="text" value="${escapeHtml(entryUserId(entry))}" required>
              </label>
              <label>
                <span>Status</span>
              ...
              <label>
                ...
                <input name="start_date" type="date" value="${escapeHtml(entryStartDate(entry))}" required>
              </label>
              <label>
                <span>End Date</span>
                <input name="end_date" type="date" value="${escapeHtml(entryEndDate(entry))}" required>
              </label>
              <label>
                <span>Start Time</span>
                <input name="start_time" type="time" value="${escapeHtml(entryStartTime(entry))}">
              </label>
              <label>
                <span>End Time</span>
                <input name="end_time" type="time" value="${escapeHtml(entryEndTime(entry))}">
              </label>
              <label>
              ...
              <label class="editor-note">
                <span>Note</span>
                <textarea name="note" rows="4">${escapeHtml(firstValue(entry.note, ''))}</textarea>
              </label>
            </div>
            <div class="editor-actions">
              ${isExisting ? `<button class="danger-btn" type="button" data-editor-delete="${escapeHtml(entry.id)}">Delete</button>` : ''}
              <span class="editor-action-spacer"></span>
              <button class="nav-btn" type="button" data-editor-close>Cancel</button>
